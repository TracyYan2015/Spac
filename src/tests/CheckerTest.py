...
class CheckerTest(unittest.TestCase):
    def testCheckerUntil(self):
        ...
        ltl = ['U[0,3600]', 'T', 'down']
        checker = Checker(model = model, ltl=ltl, duration=HOUR, checkingType=Checker.CheckingType.QUANTATIVE)
        begin = time.time()
        lowerBound, upperBound = checker.run()
        end = time.time()
        logging.info('Time caused to check the prob satisfying %s: %ss' % (str(end-begin), str(ltl)))
