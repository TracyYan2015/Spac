    ...
    def mc2(self):
        ...
        for i in range(sz):
            ...
            if isinstance(satisfied, bool):
                ...
                if satisfied:
                    satisfying += 1
                    likelihood = self.model.probForPath(
                        path, False) / self.model.probForPath(path, True)
            ...
            if verified:
                satisfying += 1
                likelihood = self.model.probForPath(
                    path, biasing=False, duration=self.duration) / self.model.probForPath(path, biasing=True, duration=self.duration)
                # logging.info('x %.10e added with %.10e' % (x, likelihood))
                x += likelihood
                # logging.info("x = %.10e" % (x))
            # logging.info('-------------\n\n')
