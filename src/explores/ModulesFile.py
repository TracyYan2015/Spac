...
class ModulesFile(object):
    ...
    # in other allUp state(not initial states), it equals to duration-passedTime.
    def nextState(self, enabledCommands, duration=0.0):
        ...
        exitRate = sum([command2.prob for command2 in enabledCommands])
        probs = [
            command.biasingRate /
        ...
        for index, prob in enumerate(probs):
            probSum += prob
            if probSum >= rnd:
                if self.initCondition(self.localVars, self.constants):
    ...
    def genRandomPath(self, duration, cachedPrefixes=None):
        ...
        while timeSum < duration:
            ...
            # check in cachedPrefixes
            if len(strPath):
                strPath += ','
            strPath += step.asKey()

            # if cachedPrefixes.has_key(strPath):
            #     transition, holdingTime, rate, biasingRate, exitRate, biasingExitRate = self.nextState()
            #     passedTime += holdingTime
            #     step = Step(self.curState.stateId, self.curState.apSet, holdingTime, passedTime, transition, rate, biasingRate, exitRate, biasingExitRate)
            #     path.append(step)
            #     self.restoreStateId()
            #     self.restoreInitState()
            #     return (cachedPrefixes[strPath], path)

            if len(path) >= 1 and self.stopCondition(
                    self.localVars, self.constants):
                # transition, holdingTime, rate, biasingRate, exitRate, biasingExitRate = self.nextState()
                # passedTime += holdingTime
                # step = Step(self.curState.stateId, self.curState.apSet, holdingTime, passedTime, transition, rate,
                #             biasingRate, exitRate, biasingExitRate)
                # path.append(step)
    ...
    def prepareCommands(self):
        # make a copy of system's variables instance
        # to not modify the variables instance.
        vsCpy = OrderedDict()
        for k, v in self.localVars.items():
            vsCpy[k] = copy.copy(v)
        for vsList in itertools.product(
                *[v.allVarsList() for _, v in vsCpy.items()]):
            vsDict = OrderedDict()
            for v in vsList:
                vsDict[v.name] = v
    ...
    def _fillInStates(self):
        for vars in itertools.product(
            ...
            if self.initCondition(dictVars, self.constants):
                self._I.add(''.join([str(var.getValue()) for var in vars]))
            elif self.failureCondition(dictVars, self.constants):
    ...
    def q1(self):
        if not self.commPrepared:
            self.prepareCommands()
            self.commPrepared = True
        key = ''.join([str(var.getValue()) for var in self.initLocalVars.values()])
...
def GYNCTMCModel():
    ...
    def down(vs, cs):
        ...
        m = vs['m'].getValue()
        d = vs['d'].getValue()
        return i == 0 or count == (MAX_COUNT +
                                   1) or o == 0 or a == 0 or m == 0 or d == 0
...
def BalancedModel():
    def failure(vs, cs):
        # true when any one kind of component failures
        return (vs['i'] == 0 and vs['di'] == 0) or (vs['a'] == 0 and vs['da'] == 0)

    def up(vs, cs):
        return not failure(vs, cs)
    ...
    def allUpOrFailure(vs, cs):
        ...
        global failureCnt
        # all components up
        if i == 1 and di == 1 and a == 1 and da == 1:
            allUpCnt += 1
            return True
    ...
    def allUp(vs, cs):
        return vs['di'] == 1 and vs['i'] == 1 and vs['a'] == 1 and vs['da'] == 1
    # using existing modules to build a balanced system with components that
    # only constains two kinds of states, e.g. up and down.
    modules = []
    modules.append(Module.AFS600())
    modules.append(Module.dAFS600())
    modules.append(Module.AT7910())
    modules.append(Module.dAT7910())
    labels = dict()
    labels['up'] = up
    ...
    model = ModulesFile(
        ...
        initCondition=allUp,
        stopCondition=allUpOrFailure,
        modules=modules,
        labels=labels)
    return model


# construct a system that only contains one component
# thus it's a balanced system
# the model can be found in 050.pdf
def toyModel():
    modules = []
    modules.append(Module.ToyModule())
    labels = dict()
    def up(vs, cs):
        return vs['v'] == 1
    def failure(vs, cs):
    ...
    labels['failure'] = failure
    labels['allUp'] = up
    model = ModulesFile(ModelType.CTMC, failureCondition=failure, initCondition=up, modules=modules, labels=labels)
    return model
