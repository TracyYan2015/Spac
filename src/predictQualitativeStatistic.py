...
class State():
    def __init__(self, id, postStates, ap):
        #need to be checked
        self.id = id
        # preStates = preStates
...
# pts needn't be passed as parameter
# so leave it out.
# returns the set of satisfied state's id
def verify(ap_path, ltl, root):
	return root in _rverify(root, ap_path, ltl, None)
...
def _rverify(root, path, ltl, pts=None):
    if root > len(ltl)-1:
        return None
    elif ltl[root] == '&':  #conjunction
        lstates = _rverify(root*2+1, path, ltl, pts)
        rstates = _rverify(root*2+2, path, ltl, pts)
        ...
        if lstates is not None and rstates is not None:
			...
			states, min_rstate = checkU(lstates, rstates,path)
			if min_rstate > steps:
        ...
        lstates = _rverify(root*2+1, path, ltl, pts)
        return checkX(lstates, path)
    elif ltl[root] == 'T':
    	# the True situation
    	return range(len(path))
    else:
        return verifyAP(root, path, ltl)

# movedtochecker.py
# lstates, rstates: set of states that satisfy y1 and y2
def checkU(lstates, rstates, path):
	print "checkU..."
	print "lstates:"
	print str(lstates)
	print "rstates:"
...
# smart get random path method
# pts: model(list of states, with the first elements be the # of states)
# k: path length
# decided_prefixes: map of shortest decidable prefixes
# def get_random_path(pts, k, decided_prefixes):
# 	# list of ap of each state
# 	path = list() 
# 	isSat = None
# 	# get the initial state id
# 	pid = pts[1].id
# 	# append the first state's ap to path
# 	path.append(pts[1].ap)
# 	for i in range(k-1):
# 		rnd = random.random()
#         sum = 0.0
#         if (len(pts[pid].postStates) == 0): return (path,isSat)
#         for j in range(len(pts[sid].postStates)):
#             sum += pts[sid].postStates[j][1]
#             if(sum >= rnd):
#                 sid = pts[sid].postStates[j][0]
#                 break
#         path.append(pts[sid].ap)
#         if str(path) in decided_prefixes:
#         	return (path, decided_prefixes[path])
#     return (path, isSat)
